...
function Write-Title {
  Write-Host;
  Write-Host "  __  __                _ ___                 "
  Write-Host " |  \/  |__ _ __ _ __ _(_) _ \__ ___ __ ___ _ "
  Write-Host " | |\/| / _` / _` / _` | |   / _` \ V  V / '_|"
  Write-Host " |_|  |_\__,_\__, \__, |_|_|_\__,_|\_/\_/|_|  "
  Write-Host "             |___/|___/                       $(Format-Text "v0.13.0.713" -Foreground Magenta)"
...
if (-not $script:CUSTOM_LICENSE) {
  do {
    $licenseeInput = Read-Host "Enter the licensee name"
    if ([string]::IsNullOrWhiteSpace($licenseeInput)) {
      Write-Warn "Licensee name cannot be empty."
    }
    else {
      $script:CUSTOM_LICENSE = $true
      $script:licensee = $licenseeInput.Trim()
      $script:license_type = "Single User License"
    }
  } while (-not $script:CUSTOM_LICENSE)
}

Invoke-OcwrLicensing
New-Toast -ToastTitle "WinRAR licensed successfully" -ToastText "You can now use the full version of WinRAR."
